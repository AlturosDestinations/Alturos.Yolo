...
//	load_weights(&net_glb, "yolov2-tiny-voc.weights");
...
int tr_context_new(char *cfgfile, char *name_list, char *weightfile)
{
	...
	//names_glb = get_labels("coco.names");
	////image **alphabet = load_alphabet();
	//net_glb = parse_network_cfg_custom("yolov2.cfg", 1); // set batch=1
	//load_weights(&net_glb, "yolov2.weights");
	//layer l = net_glb.layers[net_glb.n - 1];
	//classes_glb = l.classes;

//	names_glb = get_labels("coco.names");
//	//image **alphabet = load_alphabet();
//	net_glb = parse_network_cfg_custom("yolov2-tiny-voc.cfg", 1); // set batch=1
//	load_weights(&net_glb, "yolov2.weights");
//	layer l = net_glb.layers[net_glb.n - 1];
//	classes_glb = l.classes;
...
int tr_process_image(const uint8_t* data, const size_t data_length, RectangleCandidateContainer &container)
{
	...
	for (size_t i = 0; i < 10; ++i)
	{
		...
		//container.candidates[i].objectType = cal_container.candidates[i].objectType;
		for (int z = 0; z < 50; z++)
		{
